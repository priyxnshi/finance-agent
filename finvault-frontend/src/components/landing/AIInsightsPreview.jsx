...
export default function AIInsightsPreview() {
  return (
    <section id="ai-agent" className="border-t border-line-light dark:border-white/[0.06] py-24 transition-colors duration-150">
      <div className="max-w-6xl mx-auto px-5 lg:px-8 grid lg:grid-cols-2 gap-14 items-center">
        <div className="text-ledger-light-primary dark:text-ledger-dark-primary">
        ...
        <div className="rounded-card border border-line-light dark:border-white/10 bg-paper-raised dark:bg-ink-900 shadow-panel overflow-hidden transition-colors duration-150">
          <div className="flex items-center justify-between px-5 py-4 border-b border-line-light dark:border-white/[0.06] text-ledger-light-primary dark:text-ledger-dark-primary">
            ...
            <span className="text-2xs px-2 py-0.5 rounded-full bg-vault/10 text-vault-dim dark:text-vault-light font-medium">Live</span>
          </div>
          ...
          <div className="p-5 space-y-3">
            {insights.map(({ icon: Icon, tone, text }) => (
              <div
                key={text}
                className="flex items-center gap-3 rounded-md border border-line-light dark:border-white/10 bg-paper dark:bg-ink-850 px-4 py-3 transition-colors duration-150"
              >
                <div className={`h-8 w-8 rounded-md grid place-items-center shrink-0 ${tone}`}>
                  <Icon size={15} />
                </div>
                <p className="text-sm text-ledger-light-primary dark:text-ledger-dark-primary leading-snug">{text}</p>
              </div>
            ))}
          </div>
        </div>
