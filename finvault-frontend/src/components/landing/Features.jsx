      ...
      <div className="max-w-6xl mx-auto px-5 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {features.map(({ icon: Icon, title, description, accent }) => (
            <div
              key={title}
              className="group relative rounded-card border border-white/10 bg-ink-900 p-5 overflow-hidden hover:border-vault/30 transition-colors"
            >
              ...
              <p className="mt-1.5 text-sm text-ledger-dark-secondary leading-relaxed">{description}</p>
            </div>
          ))}
        </div>
      </div>
