...
export default function HowItWorks() {
  return (
    <section id="how-it-works" className="border-t border-white/[0.06] py-24 bg-ink-900/40">
      <div className="max-w-6xl mx-auto px-5 lg:px-8">
        <div className="max-w-xl mb-16">
          ...
          <p className="mt-4 text-ledger-dark-secondary leading-relaxed">
        ...
        <div className="hidden lg:flex items-stretch justify-between gap-2">
          {steps.map((step, i) => (
            <React.Fragment key={step.label}>
              ...
              {i < steps.length - 1 && (
                <div className="flex items-center shrink-0 text-ledger-dark-tertiary">
                  <ArrowRight size={18} />
                </div>
              )}
            </React.Fragment>
        ...
        <div className="flex lg:hidden flex-col items-center gap-2">
          {steps.map((step, i) => (
            <React.Fragment key={step.label}>
              ...
              {i < steps.length - 1 && <ArrowDown size={18} className="text-ledger-dark-tertiary" />}
            </React.Fragment>
          ))}
        </div>

        <div className="flex items-center justify-center gap-2 mt-6 text-2xs text-ledger-dark-tertiary">
          <RefreshCw size={12} />
...
function StepCard({ step, index, className = '' }) {
  ...
  return (
    <div className={`relative flex-1 rounded-card border border-white/10 bg-ink-900 p-5 ${className}`}>
      <span className="absolute top-4 right-4 text-2xs font-mono text-ledger-dark-tertiary">0{index + 1}</span>
      ...
      <p className="mt-1 text-xs text-ledger-dark-secondary leading-relaxed">{description}</p>
    </div>
  )
}
